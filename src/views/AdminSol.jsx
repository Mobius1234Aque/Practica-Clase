      ...
      async onOk() {
        try {
          const respuesta = await axios.get(
            `http://localhost:3000/registroSolAcep?curp=${record.curp}`
          );
  ...
  const handleRechazar = async (record) => {
    confirm({
      ...
      async onOk() {
        try {
          const respuesta = await axios.get(
            `http://localhost:3000/registroSolCan?curp=${record.curp}`
          );
          const datos = respuesta.data;
  ...
  const renderRegistros = () => {
    ...
    return registrosPaginados.map((registro, index) => (
      ...
      >
        ...
        >
          <p>
            <strong>CURP:</strong> {registro.curp}
          </p>
          {registro.showDetails && (
            <>
              ...
              <div>
                ...
                {new Date(registro.fecha_solicitud).toLocaleString()}
              </div>
            </>
          )}
    ...
    <>
      ...
      <div className="boxAdmin">
        ...
        {loading ? (
          ...
          <>
            <Row gutter={[20, 20]}>{renderRegistros()}</Row>
